...
| 进程守护 | 开启后 frpc 进程异常退出时自动重启（手动停止的不会重启） |
| 日志 | 打开日志目录查看运行日志 |

### 进程守护

进程守护功能可在 frpc 进程异常退出时自动重启，支持两种模式：

- **UI 模式**：在程序界面中开启，健康检查每 3 秒运行一次，异常退出自动重启
- **服务模式**：注册 Windows 服务并开启进程守护后，服务在后台持续运行并监控 frpc 进程

UI 模式和服务模式通过 `conf/guard_stopped.json` 文件协调：UI 手动停止的配置会写入此文件，服务守护读取后不会重启这些配置。

### Windows 服务

注册 Windows 服务后，每次开机将自动启动所有设置了 **自启动** 的 frpc 配置，未设置自启动的配置不会自动启动。若开启了 **进程守护**，服务将持续运行并监控 frpc 进程。
